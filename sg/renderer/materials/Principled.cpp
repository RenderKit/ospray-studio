...
{
  createChild(
      "baseColor", "rgb", "diffuse reflectivity (or metal color)", vec3f(0.8));
  createChild(
      "edgeColor", "rgb", "grazing edge tint (metallic only)", vec3f(1.f));
  ...
  createChild("transmissionColor",
      "rgb",
      "attenuated color in transmission",
      vec3f(1.f));
  createChild(
  ...
  createChild("emissiveColor", "rgb", "emissive color", vec3f(0.f));
  createChild("intensity", "float", "emissive intensity", 1.f);
}
