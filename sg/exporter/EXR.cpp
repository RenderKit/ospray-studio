...
namespace ospray {
  ...
  void EXRExporter::doExportAsLayers()
  {
    ...
    if (_geomData != nullptr && _instData != nullptr) {
      exrHeader.channels().insert("uuid.geom", Imf::Channel(IMF::UINT));
      exrHeader.channels().insert("uuid.inst", Imf::Channel(IMF::UINT));
