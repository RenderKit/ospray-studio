// copied MainWindow.h and use "devel" namespace
...
class MultiWindows : public StudioContext
{
 public:
  ...
  MultiWindows(StudioCommon &studioCommon);

  ~MultiWindows();

  static MultiWindows *getActiveWindow();

  GLFWwindow* getGLFWWindow();

  void registerDisplayCallback(std::function<void(MultiWindows *)> callback);

  void registerKeyCallback(
      std::function<void(
          MultiWindows *, int key, int scancode, int action, int mods)>);
 ...
 protected:
  ...
  static MultiWindows *activeWindow;
  ...
  std::function<void(MultiWindows *)> displayCallback;
