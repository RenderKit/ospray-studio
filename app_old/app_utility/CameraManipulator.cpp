...
namespace ospray {
  ...
  namespace imgui3D {
    ...
    InspectCenter::InspectCenter(ImGui3DWidget *widget)
        : Manipulator(widget), pivot(ospcommon::center(widget->worldBounds))
    {
    }
    ...
    void InspectCenter::rotate(float du, float dv)
    {
      ...
      AffineSpace3fa xfm = AffineSpace3fa::translate(pivot) *
                                   ...
                                   const vec2i &from)
    {
      ...
      AffineSpace3fa xfm =
          ...
          AffineSpace3fa::translate(-1.0f * widget->motionSpeed * du *
                                    cam.frame.l.vx);
                                 ...
                                 const vec2i &from)
    {
      ...
      if (widget->upAnchored) {
        const float theta =
            std::acos(dot(cam.up, normalize(cam.from - cam.at)));
